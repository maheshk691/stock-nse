...
export class NseIndia {
    ...
    /**
     * Get EPS data for a symbol
     * @param symbol NSE equity symbol
     * @returns EPS data for the symbol
     */
    async getEPSData(symbol: string): Promise<EPSData[]> {
        const corporateInfo = await this.getEquityCorporateInfo(symbol.toUpperCase())
        const epsData: EPSData[] = []

        if (corporateInfo.financial_results?.data) {
            corporateInfo.financial_results.data.forEach((result: any) => {
                epsData.push({
                    symbol: symbol.toUpperCase(),
                    eps: result.reDilEPS ? parseFloat(result.reDilEPS) : null,
                    ...
                    broadcastTimestamp: result.re_broadcast_timestamp
                })
            })
        }
