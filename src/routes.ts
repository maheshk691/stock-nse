...
/**
 * @openapi
 * /api/equity/eps/{symbol}:
 *   get:
 *     description: To get EPS data for the NSE symbol
 *     tags:
 *       - Equity
 *     produces:
 *       - application/json
 ...
 *     responses:
 *       200:
 *         description: Returns EPS data for the NSE symbol
 *       400:
 *         description: Returns a JSON error object of API call
 */
mainRouter.get('/api/equity/eps/:symbol', async (req, res) => {
    try {
        res.json(await nseIndia.getEPSData(req.params.symbol))
    } catch (error) {
        res.status(400).json(error)
    }
})
